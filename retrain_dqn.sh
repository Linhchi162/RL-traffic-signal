#!/bin/bash
# retrain_dqn.sh — DQN only: 8 seeds x 3 rewards = 24 jobs
...
echo ""
echo ">>> Xoa DQN experiments cu..."
for seed in "${SEEDS[@]}"; do
    for reward in "${REWARDS[@]}"; do
        rm -rf "$EXP_DIR/dqn_${reward}_s${seed}"
    done
done

echo ">>> Launch 24 DQN jobs (n_envs=$N_ENVS)..."
for seed in "${SEEDS[@]}"; do
    for reward in "${REWARDS[@]}"; do
...
echo ""
echo "  DQN done: $((TOTAL - FAIL))/$TOTAL OK"
