#!/bin/bash
# instance_cologne8_real_all.sh
# Setup + Train DQN + DDQN + PPO tren Cologne8 voi luu luong that (cologne8.rou.xml)
#
# Lam luon:
#   1. Cai SUMO + Python deps (neu chua co)
#   2. Git pull ban moi nhat
#   3. Download net/route Cologne8 (neu chua co)
#   4. Chay 72 jobs (3 algo x 8 seeds x 3 rewards) cung luc
#
# Usage:
#   bash <(curl -s https://raw.githubusercontent.com/Linhchi162/RL-traffic-signal/main/instance_cologne8_real_all.sh)
# hoac:
#   cd /workspace/rl-traffic && bash instance_cologne8_real_all.sh
...
# ── 4. Download Cologne8 net + route ─────────────────────────────────────────
echo "=== [4/4] Download Cologne8 (neu chua co) ==="
RESCO="https://raw.githubusercontent.com/Pi-Star-Lab/RESCO/main/resco_benchmark/environments/cologne8"
NET="$WORK_DIR/nets/cologne8/cologne8.net.xml"
ROUTE="$WORK_DIR/nets/cologne8/cologne8.rou.xml"
mkdir -p "$WORK_DIR/nets/cologne8"

if [ ! -f "$NET" ]; then
    wget -q "$RESCO/cologne8.net.xml" -O "$NET" && echo "    cologne8.net.xml OK"
else
    echo "    cologne8.net.xml da co"
fi
if [ ! -f "$ROUTE" ]; then
    wget -q "$RESCO/cologne8.rou.xml" -O "$ROUTE" && echo "    cologne8.rou.xml OK"
else
    echo "    cologne8.rou.xml da co"
fi

# ── 5. Tao danh sach 72 jobs ─────────────────────────────────────────────────
EXP_DIR="$WORK_DIR/exp_cologne8_real"
LOG_DIR="$WORK_DIR/logs_cologne8_real_all"
STEPS=500000
SIM_DUR=3600
SUMO_BEGIN=25200   # 7:00 AM tuyet doi trong cologne8.rou.xml (TAPAS Cologne)
...
OK=$(( TOTAL - FAIL ))
echo "=========================================="
echo "  cologne8-real ALL done: $OK/$TOTAL OK"
echo "  Models: $EXP_DIR/"
echo "=========================================="
