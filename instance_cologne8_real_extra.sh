#!/bin/bash
# instance_cologne8_real_extra.sh
# Chay them 2 seeds (1234, 5678) x 3 rewards x 3 algos = 18 jobs
# Ket qua luu vao exp_cologne8_real/ cung thu muc voi 8 seeds cu
#
# Usage: cd /workspace/rl-traffic && bash instance_cologne8_real_extra.sh
...
EXP_DIR="./exp_cologne8_real"
LOG_DIR="./logs_cologne8_real_all"
STEPS=500000
SIM_DUR=3600
SUMO_BEGIN=25200
SEEDS=(1234 5678)
...
[ ! -f "$NET" ] && wget -q "$RESCO/cologne8.net.xml" -O "$NET"
[ ! -f "$ROUTE" ] && wget -q "$RESCO/cologne8.rou.xml" -O "$ROUTE"

declare -a ALL_PIDS=() ALL_LABELS=()

echo ">>> Cologne8 real — 2 extra seeds: ${SEEDS[*]}"
